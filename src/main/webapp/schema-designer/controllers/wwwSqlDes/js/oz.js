...
var OZ = {
	...
	JavaRequest:function(url, callback, options) {
		...
		xhr.onreadystatechange = function() {
			...
			callback(data,xhr.status,headers,xhr.responseText);
		};
		if (o.method == "POST") { xhr.setRequestHeader("Content-Type","application/x-www-form-urlencoded"); }
		xhr.send('___action='+o.action+'&___data='+o.data+'&___keyword='+o.keyword+'&___dbProjectId='+o.dbProjectId);
		return xhr;
	},
	Request:function(url, callback, options) {
		var o = {data:false, method:"post", headers:{}, xml:false};
		for (var p in options) { o[p] = options[p]; }
		o.method = o.method.toUpperCase();
...
function performLoad(callback, keyword, dbProjectId, url) {
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4) {
			if (xmlhttp.status == 200) {
				...
				alert("Error during the call (" + xmlhttp.status
						+ "). Please try again");
			}
		}
	};
	...
	xmlhttp.send("___action=load&___data=false&___keyword="+keyword+'&___dbProjectId='+dbProjectId);
}
